...
@Component
public class VerifyMemberProcessor implements Processor {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        try (Connection conn = dataSource.getConnection()){
            ...
            UpdateConditionStep step = ctx.update(MEMBER)
                    .set(MEMBER.IS_VERIFIED, MemberIsVerified.Y)
                    .where(MEMBER.MEMBER_ID.eq(memberId));
            logger.info("verified member SQL: "+step);
            int result = step.execute();
            logger.info("verified result: "+result);
